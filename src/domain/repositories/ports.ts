// ══════════════════════════════════════════════════════════════════════
// PUERTOS (Ports) — Arquitectura Hexagonal
// Las interfaces definen el contrato. La infraestructura las implementa.
// El dominio y la aplicación NUNCA importan la implementación concreta.
// ══════════════════════════════════════════════════════════════════════
import { User, PublicUser } from '../entities/User';
import { Product, ProductWithMeta, Category } from '../entities/Product';
import { Order, OrderItem, OrderStatus, Cart, CartItem, CartItemWithProduct, Review, Coupon } from '../entities/index';
...
export interface CartSummary {
  cart_id: number; items: CartItemWithProduct[]; total: number; item_count: number;
}
...
export interface CheckoutData {
  userId: number; shipping_address: string;
  payment_method?: string; coupon_code?: string;
}

export interface IOrderRepository {
  checkout(data: CheckoutData, coupon: Coupon | undefined): { order: Order; items: OrderItem[] };
  findByUser(userId: number): (Order & { items: OrderItem[] })[];
  findAll(status?: string, page?: number, limit?: number): PaginatedResult<Order & { user_name: string; user_email: string }>;
  findById(id: number): (Order & { user_name: string; user_email: string; items: OrderItem[] }) | undefined;
