...
// ─── Init DB + auto-seed ──────────────────────────────────────────────────────
function autoSeed(db: ReturnType<typeof getDb>): void {
  ...
  const getCat = (slug: string) => (db.prepare('SELECT id FROM categories WHERE slug=?').get(slug) as { id: number } | undefined)?.id;
  ...
  products.forEach(p => {
    ...
    p.cats.forEach(slug => { const cid = getCat(slug); if (cid) insPC.run(prod.id, cid); });
  });
