...
db.prepare('INSERT OR IGNORE INTO users (name,email,password,role) VALUES (?,?,?,?)').run('Administrador KOVA', 'admin@kova.co', bcryptjs_1.default.hashSync('Admin2026!', 10), 'admin');
db.prepare('INSERT OR IGNORE INTO users (name,email,password,role) VALUES (?,?,?,?)').run('María García', 'maria@example.com', bcryptjs_1.default.hashSync('Cliente123', 10), 'client');
db.prepare('INSERT OR IGNORE INTO users (name,email,password,role) VALUES (?,?,?,?)').run('Carlos Pérez', 'carlos@example.com', bcryptjs_1.default.hashSync('Cliente123', 10), 'client');
...
prods.forEach(p => {
    ...
    p.cats.forEach(s => { const cid = getCatId(s); if (cid)
